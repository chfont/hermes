...
impl<'c> PreparedStatements<'c> {
	...
	if let Err(e) = list_stmt {
	    let fmt_str = format!("Failed to prepare list_stmt {}", e);
	...
	let insert_stmt = conn.prepare("INSERT INTO reminder (frequency, message, month, day, year, hour, minute,\
					n) VALUES (?,?,?,?,?,?,?,?)").unwrap();
	let delete_stmt = conn.prepare("DELETE FROM reminder WHERE id = ?").unwrap();
    ...
    pub fn list(&mut self, mut log: &File) -> Option<Vec<Reminder>> {
	let reminders = self.list_statement
	    .query_map([], |row: &rusqlite::Row | -> Result<Reminder, Error> {
	...
	let mut rem_vec: Vec<Reminder> = Vec::new();
